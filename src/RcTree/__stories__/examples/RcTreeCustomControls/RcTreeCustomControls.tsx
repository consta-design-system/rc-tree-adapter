...
const changeVisisble = (key: string, nodes: NodeType[]): NodeType[] => {
  return nodes.map(node => {
    if (node.key === key) {
      node.visible = !(typeof node.visible === 'undefined' ? true : node.visible)
    } else {
      if (node.children) {
...
const customNode = (params: NodeType & { handleClick: (key: string) => void }) => {
  const { title, key, visible = true, children, disableCheckbox, handleClick } = params
  return (
    <TreeNode
      key={key}
      title={
        <CustomLabel treeKey={key} title={title} handleClick={handleClick} visible={visible} />
      }
